...
Según la información proporcionada por la empresa, el sistema de suministro de aire está compuesto por un de desplazamiento positivo tipo lóbulos rotativos, marca Repicky, modelo R3.0 con una capacidad de $1631 \frac{m^{3}}{h}$. A este soplador se le acopla un motor con una potencia de 75 Hp.
...
Cabe resaltar que la información del fabricante no coincide en su totalidad con la proporcionada por la empresa, debido a las tolerancias de $\pm 5\%$ especificadas por el fabricante.
...
Para dimensionar los diámetros de las tuberías nos basamos en la Norma ASTMF-714:2012 de Tuberías lisas HDPE, el diagrama e información del sistema de aireación proporcionado por la industria y los datos del fabricante Repicky.
...
Suponiendo que el aire se distribuye de manera uniforme en las 12 ramificaciones en T de los difusores, realizamos un planteo análogo al anterior que nos da como resultado:
...
Por último, definimos el límite de salida que representa la pileta con el agua residual, cuyas características se definen según los datos provistos para la temperatura y los cálculos anteriores para la presión.
...
Actualmente, para medir el OD, la empresa tiene el sensor \href{https://afcintl.com/p/model-q46d-dissolved-oxygen-monitor-from-analytical-technology/}{DissolvedOxygenMonitor Model Q46D}:
...
De igual manera, en la parte derivativa $\frac{de(t)}{dt}$ se tiene que expresar de forma tal que sea codificable. Tomando como referencia lo expuesto en \cite{STW}, se puede definir la derivada, como la pendiente de la recta tangente: 
...
Se debe tener en cuenta, que para un cálculo preciso de los conductores y protecciones, es necesario conocer en detalle la instalación eléctrica de la planta aguas arriba del tablero seccional. Al no disponer de esa información, todos los análisis posteriores tienen un enfoque de diseño simplemente.
...
Aplicando un factor de corrección de 1.25 para contemplar la intensidad de arranque:

$$I_{n} = 105A \cdot 1.25 = 131.25A$$
...
\begin{center}
    \includegraphics[scale=.48]{src/imagenes/sistema/tablaSeccionMotor.png}
\end{center}
...
\begin{itemize}
    ...
    \item{Es necesario un compilador cruzado para una arquitectura específica.}
    \item{Las versiones precompiladas no están configuradas como se necesitan.}
\end{itemize}
...
El proceso paso a paso de la configuración es demasiado largo como para incluirlo aquí; sin embargo, tenemos un \href{https://github.com/GabiAndi/Build_toolchains}{repositorio en GitHub} en donde se puede ver el desarrollo completo.

\section{Protocolo de comunicación}

\label{anexo:protocoloDeComunicacion}

Para que el controlador pueda realizar su trabajo y logre interactuar con los sistemas subyacentes, es necesario una convención en cuanto a la transmisión y recepción de datos. Con lo cual se debió optar por un protocolo de comunicación para comunicarse con el controlador del sistema, en donde se dispongan de distintos comandos, los cuales están en formato Little Endian y poseen la siguiente estructura:
