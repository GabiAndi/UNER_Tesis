...
Corresponde al OD que trae consigo el caudal de ingreso de agua residual. El mismo, se genera mediante el proceso de aireación natural, es aquella que se efectúa por contacto del agua con el aire. Cuando cambiamos la superficie de agua que entra en contacto con el aire, la sometemos a aireación en forma permanente, sin embargo, el proceso de difusión de los gases en el agua es sumamente lenta. Se difunde lentamente y su distribución depende del movimiento del agua. Este proceso es natural y continuo, de modo que continuamente existe intercambio de oxigeno entre el agua y el aire. La dirección y velocidad depende del contacto entre ambos.
...
El control del oxígeno disuelto en función a la demanda bioquímica de oxígeno, es muy complicado de determinar por sensado. Como se describió anteriormente la única forma de conocer este valor es mediante ensayos de laboratorio. Sin embargo, aún se puede establecer manualmente una serie de parámetros en el proceso que estén controlados por el sistema, pero que se determinen de forma manual. Es decir, que el setpoint de OD sea determinado por ensayos, pero el control de los sopladores y la cantidad de aire estrangulado, sea corregido en función al valor del sensor de OD. Es por esto que se abordo un desarrollo de control genérico pero altamente adaptable a los requerimientos y tiempos de respuesta del proceso. \\

Un control PID es el mas adecuado en este caso, ya que tiene las componentes proporcional, integral y derivativa. Si una de ellas no es necesaria, se puede directamente anular, pero aún sigue manteniendo la capacidad de adaptación de un control completo.

\subsection{Sistema a controlar}

Como ya hemos planteado, se propone controlar el nivel de OD en base a la cantidad de aire estrangulado de los sopladores. Esto permite que los motores no trabajen siempre a valores nominales, permitiendo un desbloqueo en el potencial de ahorro energético. \\

Se sabe de parte de una de las docentes que traba en la industria, que el tiempo de respuesta entre un cambio de caudal de aire estrangulado y el nivel de OD, es muy lento. Aproximadamente toma una hora o más para que haya una variación en el nivel del sensor. Esto plantea una situación desfavorable. Nuestro sistema de control tendrá que variar muy lentamente la frecuencia de los sopladores para poder lograr una compensación adecuada del proceso.
...
Como mencionamos anteriormente, no tuvimos la oportunidad de realizar mediciones y ensayos de prueba para poder determinar su modelo matemático aproximado y ver como este responde ante diferentes entradas. Es por esto que investigamos una relación de variación del OD respecto del tiempo, para poder así, probar distintas entradas. \\

Basándonos en el articulo académico denominado \cite{CPCOD}, sostiene que la dinámica de cambio de la concentración de OD se puede representar por:

$$\frac{\delta C_{o}}{\delta t} = K_{L} a (C_{\infty}^{\Theta} - C_{o})$$

Donde:

\begin{itemize}
    \item{$C_{o}$: concentración de oxígeno disuelto en el seno del liquido para el agua residual aproximada en $g/m^{3}$.}
    \item{$C_{\infty}^{\Theta}$: concentración de saturación del oxígeno disuelto en el volumen del líquido a una temperatura T del agua y a una presión atmosférica de campo en $g/m^{3}$.}
...
$$\frac{\delta C_{o}}{\delta t} = \frac{Q_{f}}{V} C_{of} - \frac{Q_{f} + Q_{r}}{V} C_{o} + a (1 - e^{\frac{-Q_{A}}{b}}) (C_{\infty}^{\Theta} - C_{o})$$
...
\begin{itemize}
    ...
    \item{$C_{0f}$: concentración DO en el flujo de entrada $8,23mg/l$ o $8230mg/m^3$.}
\end{itemize}

En nuestro caso considerando un sistema ideal en el cual, todo el caudal de aire suministrado por el soplador ingresa a la pileta (no se consideran perdidas) tenemos: 
...
$$\frac{dC_{0}}{dt} = {\frac {12 m^{3}/h} {871,03 m^{3}}} * 8230mg/m^3 - {\frac {12 m^{3}/h+6 m^{3}/h} {871,03 m^{3}}} * C_{0} + 8,5 *(1- e^{\frac {-{0.78 m^3/h } * RPM} {2,4767}}) (C^{\Theta }_{\infty} - C_{0})$$

\subsubsection{Controlador}

Es por esto, que se plantea el uso de un controlador PID genérico, en donde variando algún parámetro de configuración, tenemos la posibilidad de adaptarlo. \\
